...
- **VCEK air-gap path = Trustee-side OfflineStore.** (Reversed from the initial host-side-preload
  ...
  - `KbsConfig.spec.kbsLocalCertCacheSpec` mounts host VCEK secrets at
    `/opt/confidential-containers/attestation-service/kds-store/vcek/<hwid-lowercase>/vcek.der`.
  - For each eligible AMD host, use `snphost show vcek-url` → download `.der` on a connected host →
    carry it in. The procedure is identical for single- and multi-socket hosts.
