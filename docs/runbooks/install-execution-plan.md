...
| **🛑 STOP-gate** | All rung a/b/c happy+negative results green **and** the air-gap VCEK-pull negative test fails-closed as expected. For b/c sign-off, collect and validate reviewer-grade evidence with `make collect-rung-bc-evidence` and `make validate-rung-bc-evidence EVIDENCE_DIR=<bundle>`. |
...
| **CRD field names** (KataConfig/KbsConfig guesses) | `oc explain kataconfig.spec` / `kbsconfig.spec` once CSVs Succeed | correct `enableConfidentialCompute` / `kbsLocalCertCacheSpec`, re-apply |
