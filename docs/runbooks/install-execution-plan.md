...
| **Rung b — encrypted image** | `make build-rung-images` then `make deploy-trustee-rung-bc` and `make run-rung-b-encrypted RUNG_B_IMAGE=<digest-ref>`. **Happy:** pod Running (image key released after attestation from `image-key/rung-b`). **Negative:** `make negative-test WHICH=rung-b RUNG_B_IMAGE=<digest-ref>` must fail closed from a measured-initdata mismatch, not from a missing key. *(after rung a)* |
| **Rung c — signed image** | Use the same artifacts, then `make run-rung-c-signed RUNG_C_IMAGE=<digest-ref>`. **Happy:** signed image pulls (mirror pull secret served as `regcred`). **Negative:** `make negative-test WHICH=rung-c RUNG_C_UNSIGNED_IMAGE=<unsigned-digest-ref>` must fail closed through `image_security_policy` rejection. `regcred` name **without dots**; registry CA in initdata as **separate array elements**; policy must allow/verify pause/release images too. *(after rung b)* |
