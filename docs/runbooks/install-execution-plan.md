...
| **Rung b — signed image** | **Keyprovider-free path (recommended in an air gap):** `make build-rung-b` (skopeo copy + cosign sign — no `coco-keyprovider`), then `make deploy-trustee-rung-b RUNG_B_IMAGE=<digest-ref> RUNG_B_COSIGN_PUB=<pub>` and `make run-rung-b-signed RUNG_B_IMAGE=<digest-ref>`. (`make build-rung-images` also builds rung-b, but it bundles rung-c and **requires `coco-keyprovider`**, which isn't in the mirror and can't be built in-gap — see follow-ups.) **Happy:** signed image pulls (mirror pull secret served as `regcred`). **Negative:** `make negative-test WHICH=rung-b RUNG_B_UNSIGNED_IMAGE=<unsigned-digest-ref>` must fail closed through `image_security_policy` rejection. `regcred` name **without dots**; registry CA in initdata as **separate array elements**; policy must allow/verify pause/release images too. **Proven on the rig 2026-07-01 via the keyprovider-free path.** *(after rung a)* |
