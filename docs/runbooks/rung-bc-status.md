...
Last updated: 2026-06-30T13:32:27Z

Current PR: #8, `codex/rung-bc-support`
Latest proof-tooling checkpoint verified on the rig: `9c94f67` (`make summarize-rung-b-direct-pull`
from this head validated and summarized the manifest/env-aware direct-pull bundle at
`/home/rocky/occ-rung-bc-proof/rung-bc-artifacts/rung-b-direct-pull-20260630T125624Z`, which was
collected from clean head `dfae546`; `make validate-rung-c-evidence` still passes for
...
<https://github.com/cri-o/cri-o/issues/10084>, which was reopened with the latest strict
manifest/env-aware diagnostic evidence at `2026-06-30T13:17:44Z`.
...
- `make diagnose-rung-b-direct-pull` now renders the direct digest-pinned rung-b pod, waits for
  ...
  pod, event, Trustee, CRI-O, mirror context, and the `rung-bc-images.json`/`rung-bc.env`
  artifact handoff when present. It exits zero only when the blocker appears before any Trustee
  image-key request.
...
- `scripts/apply-rung-image.sh` now writes its SNO-baseline validation output under the run's
  private temp directory instead of `/tmp/apply-rung-image-baseline.log`, which avoided a rig
  replay failure when that shared path was not writable.
...
- Fresh strict rung-c replay evidence:
  `/home/rocky/occ-rung-bc-proof/rung-bc-artifacts/evidence-rung-c-strict-20260630T102013Z`
  on the bastion. It was collected from repo head `6c5536d` with `repo_git_dirty=false` and
  proof-window bounds
  `trustee_log_since_time=crio_log_since_time=mirror_log_since_time=2026-06-30T10:20:13Z`.
  Full `validate-rung-bc-evidence.sh` still exits non-zero with twelve issues caused by
  missing/incomplete rung-b direct and negative evidence, but the scoped rung-c validator passes
  this bundle: happy pod Running, pod-status app-start, same decoded initdata for the unsigned
  negative, Trustee `security-policy/rung-c` and `sig-public-key/rung-c` fetches, guest
  `oci-client` manifest/blob pulls for the signed digest, guest `oci-client` manifest pull for the
...
- Fresh `make prove-rung-c` evidence:
  `/home/rocky/occ-rung-bc-proof/rung-bc-artifacts/evidence-rung-c-proof-20260630T124321Z`.
  It was collected from clean repo head `7206098` with `repo_git_dirty=false` and
  ...
  bounded Trustee/CRI-O/mirror logs since `2026-06-30T12:43:21Z`, and
  ...
  which predates the latest direct-pull diagnostic provenance changes.
...
- The current packaged direct-pull diagnostic bundle
  ...
  validator head `9c94f67`. The validation confirms the
  ...
  - After adding direct-pull diagnostic repo provenance and strict clean-checkout validation, it
    was rerun at
    `/home/rocky/occ-rung-bc-proof/rung-bc-artifacts/rung-b-direct-pull-20260630T123552Z`.
    The diagnostic again reproduced `classification=known-host-pull-blocker`: pod phase
    `Pending`, `host_pull_blocker_seen=1`, `image_key_request_seen=0`,
    `repo_git_head=46b77cd5d80a7d546a8dfeb92c43a84143b37120`,
    `repo_git_dirty=false`, `crio_log_since_time=2026-06-30T12:35:52Z`,
    `mirror_log_since_time=2026-06-30T12:35:52Z`, `crio_rung_b_manifest=16`,
    `crio_rung_b_blob=16`, `guest_rung_b_manifest=0`, and `guest_rung_b_blob=0`.
    Running
    `make validate-rung-b-direct-pull DIAG_DIR=/home/rocky/occ-rung-bc-proof/rung-bc-artifacts/rung-b-direct-pull-20260630T123552Z`
    passed with the strict default repo-provenance, CRI-O host-pull, log-window, and
    mirror-summary requirements. The helper removed the diagnostic pod afterward; the node
    remained Ready and no debug pods were left behind.
  - After adding manifest handoff and strict manifest consistency validation, it was rerun at
    `/home/rocky/occ-rung-bc-proof/rung-bc-artifacts/rung-b-direct-pull-20260630T125624Z`.
    The diagnostic again reproduced `classification=known-host-pull-blocker`: pod phase
    `Pending`, `host_pull_blocker_seen=1`, `image_key_request_seen=0`,
    `repo_git_head=dfae54615e8eee3e22b7209da1c8b3714dceda63`,
    `repo_git_dirty=false`, `crio_log_since_time=2026-06-30T12:56:24Z`,
    `mirror_log_since_time=2026-06-30T12:56:24Z`, `crio_rung_b_manifest=16`,
    `crio_rung_b_blob=16`, `guest_rung_b_manifest=0`, and `guest_rung_b_blob=0`.
    The bundle includes copied `rung-bc-images.json` and `rung-bc.env`; strict validation confirmed
    the manifest's rung-b digest ref and KBS key ID match the diagnostic summary and that the
    manifest records a non-secret 64-hex key fingerprint. Running
    `make validate-rung-b-direct-pull DIAG_DIR=/home/rocky/occ-rung-bc-proof/rung-bc-artifacts/rung-b-direct-pull-20260630T125624Z`
    passed with the strict default artifact-manifest, repo-provenance, CRI-O host-pull, log-window,
    and mirror-summary requirements. The helper removed the diagnostic pod afterward; the node
    remained Ready and no debug pods were left behind.
...
1. Find a supported OpenShift/CRI-O path that gets direct rung-b pods to `CreateContainer` without
   ...
   upstream issue <https://github.com/cri-o/cri-o/issues/10084> is open again with the latest
   strict diagnostic evidence, and the repo-local escalation packet is in
   `docs/runbooks/rung-b-upstream-escalation.md`.
...
direct-pull blocker is tracked upstream in <https://github.com/cri-o/cri-o/issues/10084>, which
is open again with the latest strict diagnostic evidence.
