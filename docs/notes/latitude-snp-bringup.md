...
verify-first gate the whole bare-metal decision rested on.

## Hardware / image (what worked)
- Provider: **Latitude.sh**, plan **`m4-metal-medium`** = AMD **EPYC 9124 (Genoa)**, 16c / 128 GB, NYC (hourly bare-metal billing).
...
## The misleading failure mode (a known upstream issue)
...
kernel (the actual deliverable) — that's verified at the OpenShift/SNO phase. The production
hardware will also need this same BIOS sequence (a pre-deployment scoping item).
