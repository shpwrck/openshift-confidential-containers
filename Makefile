...
collect-rung-bc-evidence: ## Phase 6: collect non-secret rung-b/c proof evidence into ARTIFACT_DIR
	NS="$(WORKLOAD_NS)" TRUSTEE_NS="$(NS)" ARTIFACT_DIR="$(ARTIFACT_DIR)" EVIDENCE_DIR="$(EVIDENCE_DIR)" PODS="$(EVIDENCE_PODS)" RUNG_B_POD="$(RUNG_B_POD)" RUNG_C_POD="$(RUNG_C_POD)" NEG_RUNG_B_POD="$(NEG_RUNG_B_POD)" NEG_RUNG_C_POD="$(NEG_RUNG_C_POD)" RUNG_B_APP_LOG_MARKER="$(RUNG_B_APP_LOG_MARKER)" RUNG_C_APP_LOG_MARKER="$(RUNG_C_APP_LOG_MARKER)" TRUSTEE_LOG_TAIL="$(TRUSTEE_LOG_TAIL)" POD_LOG_TAIL="$(POD_LOG_TAIL)" MIRROR_LOG_TAIL="$(MIRROR_LOG_TAIL)" MIRROR_LOG_FILES="$(MIRROR_LOG_FILES)" MIRROR_CONTAINER_NAMES="$(MIRROR_CONTAINER_NAMES)" bash "$(COLLECT_RUNG_BC_EVIDENCE_SCRIPT)"
...
validate-rung-bc-evidence: ## Phase 6: validate a collected rung-b/c evidence bundle (set EVIDENCE_DIR)
	...
	EVIDENCE_DIR="$(EVIDENCE_DIR)" RUNG_B_POD="$(RUNG_B_POD)" RUNG_C_POD="$(RUNG_C_POD)" NEG_RUNG_B_POD="$(NEG_RUNG_B_POD)" NEG_RUNG_C_POD="$(NEG_RUNG_C_POD)" RUNG_B_APP_LOG_MARKER="$(RUNG_B_APP_LOG_MARKER)" RUNG_C_APP_LOG_MARKER="$(RUNG_C_APP_LOG_MARKER)" bash "$(VALIDATE_RUNG_BC_EVIDENCE_SCRIPT)" "$(EVIDENCE_DIR)"

.PHONY: prove-rung-bc
prove-rung-bc: ## Phase 6: run rung-b/c happy paths, denial proofs, evidence collection, and validation
	NS="$(WORKLOAD_NS)" TRUSTEE_NS="$(NS)" MIRROR_REGISTRY="$(MIRROR_REGISTRY)" MIRROR_DNS_UPSTREAM="$(MIRROR_DNS_UPSTREAM)" KBS_URL="$(KBS_URL)" ARTIFACT_DIR="$(ARTIFACT_DIR)" EVIDENCE_DIR="$(EVIDENCE_DIR)" PODS="$(EVIDENCE_PODS)" RUNG_B_POD="$(RUNG_B_POD)" RUNG_C_POD="$(RUNG_C_POD)" NEG_RUNG_B_POD="$(NEG_RUNG_B_POD)" NEG_RUNG_C_POD="$(NEG_RUNG_C_POD)" RUNG_B_APP_LOG_MARKER="$(RUNG_B_APP_LOG_MARKER)" RUNG_C_APP_LOG_MARKER="$(RUNG_C_APP_LOG_MARKER)" RUNG_B_IMAGE="$(RUNG_B_IMAGE)" RUNG_C_IMAGE="$(RUNG_C_IMAGE)" RUNG_C_UNSIGNED_IMAGE="$(RUNG_C_UNSIGNED_IMAGE)" APPLY_RUNG_B_SCRIPT="$(APPLY_RUNG_B_SCRIPT)" APPLY_RUNG_C_SCRIPT="$(APPLY_RUNG_C_SCRIPT)" NEGATIVE_TEST_SCRIPT="$(NEGATIVE_TEST_SCRIPT)" COLLECT_RUNG_BC_EVIDENCE_SCRIPT="$(COLLECT_RUNG_BC_EVIDENCE_SCRIPT)" VALIDATE_RUNG_BC_EVIDENCE_SCRIPT="$(VALIDATE_RUNG_BC_EVIDENCE_SCRIPT)" bash "$(PROVE_RUNG_BC_SCRIPT)"
