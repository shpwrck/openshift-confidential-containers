...
collect-rung-bc-evidence: ## Phase 6: collect non-secret rung-b/c proof evidence into ARTIFACT_DIR
	NS="$(WORKLOAD_NS)" TRUSTEE_NS="$(NS)" KBS_URL="$(KBS_URL)" RUNG_B_KEY_ID="$(RUNG_B_KEY_ID)" RUNG_B_POLICY_URI="$(RUNG_B_POLICY_URI)" RUNG_C_POLICY_URI="$(RUNG_C_POLICY_URI)" ARTIFACT_DIR="$(ARTIFACT_DIR)" EVIDENCE_DIR="$(EVIDENCE_DIR)" PODS="$(EVIDENCE_PODS)" RUNG_B_POD="$(RUNG_B_POD)" RUNG_C_POD="$(RUNG_C_POD)" NEG_RUNG_B_POD="$(NEG_RUNG_B_POD)" NEG_RUNG_C_POD="$(NEG_RUNG_C_POD)" RUNG_B_APP_LOG_MARKER="$(RUNG_B_APP_LOG_MARKER)" RUNG_C_APP_LOG_MARKER="$(RUNG_C_APP_LOG_MARKER)" TRUSTEE_LOG_TAIL="$(TRUSTEE_LOG_TAIL)" TRUSTEE_LOG_SINCE_TIME="$(TRUSTEE_LOG_SINCE_TIME)" POD_LOG_TAIL="$(POD_LOG_TAIL)" MIRROR_LOG_TAIL="$(MIRROR_LOG_TAIL)" MIRROR_LOG_SINCE_TIME="$(MIRROR_LOG_SINCE_TIME)" MIRROR_LOG_FILES="$(MIRROR_LOG_FILES)" MIRROR_CONTAINER_NAMES="$(MIRROR_CONTAINER_NAMES)" bash "$(COLLECT_RUNG_BC_EVIDENCE_SCRIPT)"
...
diagnose-rung-b-direct-pull: ## Phase 6: reproduce/collect the rung-b direct encrypted-image pull blocker
	NS="$(WORKLOAD_NS)" TRUSTEE_NS="$(NS)" MIRROR_REGISTRY="$(MIRROR_REGISTRY)" MIRROR_DNS_UPSTREAM="$(MIRROR_DNS_UPSTREAM)" KBS_URL="$(KBS_URL)" RUNG_B_KEY_ID="$(RUNG_B_KEY_ID)" RUNG_B_POLICY_URI="$(RUNG_B_POLICY_URI)" RUNG_B_IMAGE="$(RUNG_B_IMAGE)" ARTIFACT_DIR="$(ARTIFACT_DIR)" MIRROR_LOG_TAIL="$(MIRROR_LOG_TAIL)" MIRROR_LOG_SINCE_TIME="$(MIRROR_LOG_SINCE_TIME)" MIRROR_LOG_FILES="$(MIRROR_LOG_FILES)" MIRROR_CONTAINER_NAMES="$(MIRROR_CONTAINER_NAMES)" bash "$(DIAGNOSE_RUNG_B_DIRECT_PULL_SCRIPT)"
