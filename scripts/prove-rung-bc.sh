...
KBS_URL="$KBS_URL" RUNG_B_IMAGE="$RUNG_B_IMAGE" bash "$APPLY_RUNG_B_SCRIPT"

echo
echo "== rung-c happy path =="
KBS_URL="$KBS_URL" RUNG_C_IMAGE="$RUNG_C_IMAGE" bash "$APPLY_RUNG_C_SCRIPT"

echo
echo "== rung-b fail-closed proof =="
KBS_URL="$KBS_URL" RUNG_B_IMAGE="$RUNG_B_IMAGE" KEEP_DENIED_PODS=1 bash "$NEGATIVE_TEST_SCRIPT" rung-b

echo
echo "== rung-c fail-closed proof =="
KBS_URL="$KBS_URL" RUNG_C_UNSIGNED_IMAGE="$RUNG_C_UNSIGNED_IMAGE" KEEP_DENIED_PODS=1 bash "$NEGATIVE_TEST_SCRIPT" rung-c

echo
echo "== collect rung-b/c evidence =="
EVIDENCE_DIR="$EVIDENCE_DIR" KBS_URL="$KBS_URL" RUNG_B_KEY_ID="$RUNG_B_KEY_ID" PODS="$PODS" RUNG_B_POD="$RUNG_B_POD" \
	...
	TRUSTEE_LOG_TAIL="$TRUSTEE_LOG_TAIL" POD_LOG_TAIL="$POD_LOG_TAIL" MIRROR_LOG_TAIL="$MIRROR_LOG_TAIL" \
...
KBS_URL="$KBS_URL" RUNG_B_KEY_ID="$RUNG_B_KEY_ID" RUNG_B_POD="$RUNG_B_POD" RUNG_C_POD="$RUNG_C_POD" \
