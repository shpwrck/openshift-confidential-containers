...
#
# Usage: ./scripts/negative-test.sh [all|rung-kbs|rung-rvps|rung-signed|rung-encrypted|air-gap]
#   Capability rungs (docs present the ladder as A→B→C→D): rung-kbs=A, rung-rvps=B (measurement),
#   rung-signed=C, rung-encrypted=D. `all` runs kbs+rvps+signed+air-gap; rung-encrypted (D) is manual.
# Env: NS=default  TRUSTEE_NS=trustee-operator-system  TIMEOUT=120  KEEP_DENIED_PODS=0
#      MIRROR_REGISTRY=mirror.rig.local:8443  RUNG_ENCRYPTED_IMAGE=...  RUNG_SIGNED_UNSIGNED_IMAGE=...
#      RUNG_ENCRYPTED_POLICY_URI=...  RUNG_SIGNED_POLICY_URI=...
#      Source rung-image-artifacts/rung-image.env after make build-rung-images for digest refs.
set -euo pipefail
...
RUNG_ENCRYPTED_POLICY_URI="${RUNG_ENCRYPTED_POLICY_URI:-kbs:///default/security-policy/test}"
...
# rung-kbs is SELF-CONTAINED (mirrors the air-gap swap-and-restore): the base attestation-policy is
...
# sha256 of the exact initdata bytes; the rung-kbs secret released ONLY when configuration is affirmed),
# then:
...
run_rung_encrypted() {
  echo "[rung-encrypted] encrypted image — tamper measured initdata so image key is withheld"
  local manifest
  manifest="$(mktemp)"
...
run_rung_signed() {
  echo "[rung-signed] signed image — use unsigned/tampered image so image_security_policy rejects"
  local unsigned_image manifest
  unsigned_image="${RUNG_SIGNED_UNSIGNED_IMAGE:-${MIRROR_REGISTRY}/coco/rung-b-unsigned:unsigned}"
  manifest="$(mktemp)"
...
run_air_gap() {
  ...
  restart_kbs
  sleep 5
  if render_or_skip "air-gap happy-path rung-kbs manifest" "$manifest" \
      env NS="$NS" TRUSTEE_NS="$TRUSTEE_NS" MIRROR_REGISTRY="$MIRROR_REGISTRY" \
        MIRROR_DNS_UPSTREAM="$MIRROR_DNS_UPSTREAM" KBS_URL="$KBS_URL" \
        POD_NAME=negtest-air-gap RENDER_ONLY=1 \
        bash "$REPO_ROOT/scripts/apply-rung-kbs.sh"; then
    expect_fail_closed "negtest-air-gap" "$manifest" "air-gap wrong-VCEK denied otherwise happy rung-kbs" "$AIR_GAP_DENIAL_RE" "air-gap VCEK/OfflineStore denial"
  fi
  air_gap_restore
  rm -rf "$bakdir"
  rm -f "$manifest"
...
case "$WHICH" in
  rung-kbs)       run_rung_kbs ;;
  rung-rvps)      run_rung_rvps ;;
  rung-signed)    run_rung_signed ;;
  rung-encrypted) run_rung_encrypted ;;
  air-gap)        run_air_gap ;;
  all)
    # Canonical hands-off order: kbs → rvps → signed → air-gap. rung-encrypted (D) is manual
    # (upstream-blocked cri-o/cri-o#10084) and is deliberately EXCLUDED from `all`.
    run_rung_kbs
    run_rung_rvps
    run_rung_signed
    run_air_gap ;;
  rung-a|rung-b|rung-c)
    die "retired token '$WHICH': rungs are named by capability now — rung-a→rung-kbs, rung-b→rung-signed, rung-c→rung-encrypted (measurement verification is the new rung-rvps). Re-run with the capability name." ;;
  *) die "unknown target '$WHICH' (use: all|rung-kbs|rung-rvps|rung-signed|rung-encrypted|air-gap)" ;;
esac
